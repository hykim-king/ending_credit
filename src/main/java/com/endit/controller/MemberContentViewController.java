...
public class MemberContentViewController {
	...
	/**
	 * 로그인 회원의 기록 화면과 탭별 건수 전달 상세 목록은 JavaScript에서 REST API로 조회
	 *
	 ...
	 */
	@GetMapping
	public String records(@RequestParam(defaultValue = TAB_RATINGS) String tab, Model model) {
		...
		model.addAttribute("collectionCount", collectionService.countVisibleByMember(memberId, Long.valueOf(memberId)));
